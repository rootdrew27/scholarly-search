...
class EmbeddingLibrary():
    ...
    def get_full_paper_embs(self) -> np.ndarray:
        if self.full_paper_embs == None:
            paper_embs = []
            for file in self.path_to_embs.iterdir():
                paper_embs.append(np.load(file)[-1, :]) # get the last embedding
            all_embs = np.stack(paper_embs, axis=0)
            ...
            self.full_paper_embs = all_embs
        return self.full_paper_embs

    def preprocess_llm_response(self, prompt:str, response:str) -> list[str]:
        response = response.replace(prompt, '').strip()
        response = re.sub(r'reference[s]?.*', r'', response, flags=re.IGNORECASE)
        response_secs = re.split(r'SECTION:\s\w+\n', response)
        response_secs = [rs.strip(' \n') for rs in response_secs]
        return response_secs

    def search_papers(self, prompt:str, response:str, norm_embs:bool, n_results=5):
        ...
        r_emb = self.model.encode(r_secs, normalize_embeddings=norm_embs).mean(axis=0)
        scores = self.model.similarity(r_emb, self.full_paper_embs)
