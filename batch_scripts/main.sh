...
#SBATCH --output=output/test_3.txt     # Output file - Use %j to inject job id, like output-%j.txt

#SBATCH --error=error/test_3.txt       # Error file - Use %j to inject job id, like error-%j.txt
...
#SBATCH --ntasks-per-node=6     # How many CPU cores do you want to use per node (max 64)?
...
python3.9 ./py_code/main.py "test.txt" "INFO"
