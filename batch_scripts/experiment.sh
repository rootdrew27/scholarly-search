...
#SBATCH --output=output/experiment_2.txt     # Output file - Use %j to inject job id, like output-%j.txt
#SBATCH --error=error/experiment_2.txt       # Error file - Use %j to inject job id, like error-%j.txt
...
#SBATCH --nodes=2               # How many nodes do you need to use at once?
...
python3.9 ./py_code/experiment_2.py "andrew_root.txt" "INFO"
