...
class MultipleNegativesRankingLoss(nn.Module):
    ...
    def __init__(self, model: SentenceTransformer, scale: float = 20.0, similarity_fct=util.cos_sim, kl_factor=0.5) -> None:
    ...
    def forward(self, sentence_features: Iterable[dict[str, Tensor]], labels: Tensor) -> Tensor:
        ...
        batch_means, batch_stds = embeddings_a.mean(0), embeddings_a.std(0)

        kl_loss = self.kl_factor * ((batch_means).abs().mean() + (batch_stds - 1).abs().mean())



        return self.cross_entropy_loss(scores, range_labels) + kl_loss
