...
class HTMLHandler:
    ...
    def _parse_html_content(self, html_content: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Remove math tags
            math_tags = soup.find_all(['math','cite','a','"div", {"id": "articlebody"}'])
...
def process_multiple_search_terms(
    ...
    return results
def get_paper_info(arxiv_id: str, text_directory: str = ".") -> dict:
    """
    Get basic information about a paper from its saved text file.
    ...
    Returns:
        dict: Dictionary containing paper title, link, and summary
    """
    try:
        # Construct the filename
        filename = f"{arxiv_id}_content.txt"
        filepath = os.path.join(text_directory, filename)
        
        # Check if file exists
        if not os.path.exists(filepath):
            return {
                "error": f"No text file found for arXiv ID: {arxiv_id}",
                "success": False
            }
        ...
        return {
            "error": f"Error retrieving paper info: {str(e)}",
            "success": False
        }
def paperLink(paperID):
    arxiv_link = f"https://arxiv.org/abs/{paperID}"
    pdf_link = f"https://arxiv.org/abs/{paperID}"
...
if __name__ == "__main__":
    ...
    print(paperLink(paperID="2311.00207v3"))
